...
public class ObjectRule implements Rule<JPackage, JType> {
    ...
    private void addConstructor(JDefinedClass jclass, JDefinedClass builderClass) {
        ...
        for (Map.Entry<String, JFieldVar> e : jclass.fields().entrySet()) {
            // JVar param = constructor.param(e.getValue().type(), e.getKey());
    ...
    private static JExpression immutableCopy(JExpression expr, JType type) {
        final JClass immutableType;
        String typeName = type.erasure().fullName();
        if (typeName.equals("java.util.List")) {
            immutableType = type.owner().ref(ImmutableList.class);
        } else if (typeName.equals("java.util.Set")) {
            immutableType = type.owner().ref(ImmutableSet.class);
        } else if (typeName.equals("java.util.Map")) {
            immutableType = type.owner().ref(ImmutableMap.class);
        } else {
            return expr;
        }
        return immutableType.staticInvoke("copyOf").arg(expr);
    }
    ...
    private JDefinedClass generateExternalBuilder(JDefinedClass jclass) throws JClassAlreadyExistsException {
        ...
        /*
        JInvocation buildConstructorCall = JExpr._new(jclass);
        for(Map.Entry<String, JFieldVar> e : jclass.fields().entrySet()) {
            JFieldVar field = builderClass.field(JMod.PRIVATE, e.getValue().type(), e.getKey());

            JMethod setter = builderClass.method(JMod.PRIVATE, builderClass, "with" + capitalize(e.getKey()));
            JVar param = setter.param(e.getValue().type(), e.getKey());
            JBlock body = setter.body();
            body.assign(JExpr._this().ref(field), param);
            body._return(JExpr._this());

            buildConstructorCall.arg(JExpr._this().ref(field));
        }
        */
